 ...
 * `textureRegistry`, and `videoFrameRegistry` are the seams a consumer
 * outside this package has a legitimate reason to override:
 * `textRenderRegistry` is required for any `TextNode` to render at all (see
 * `TextRenderRegistry`'s own doc - with none supplied, `buildTextObject`
 * resolves every text node to an empty, glyph-less group rather than
 * throwing, so this stays optional here too, matching that same "not yet
 * loaded is an expected runtime state" contract); `textureRegistry` is the
 * same story for an `ImageNode`'s own `assetRef` (see `TextureRegistry`'s
 * own doc - with none supplied, every image renders as its documented gray
 * placeholder plane); `videoFrameRegistry` is the same story again for a
 * `VideoNode`'s own current frame (see `VideoFrameRegistry`'s own doc -
 * with none supplied, every video renders as its documented placeholder
 * plane). Swapping the underlying Three.js renderer construction itself is
 * an internal testing seam (see `ThreeRendererDependencies` in
 ...
 * `VideoFrameRegistry` (so every `TextNode`/`ImageNode`/`VideoNode` renders
 * as its own documented placeholder - see `CreateRendererOptions`' own
 * doc).
 *
 * `ThreeRenderer`'s constructor takes `textRenderRegistry`/`textureRegistry`/
 * `videoFrameRegistry` as positional arguments after `environmentRegistry`/
 * `lutRegistry`/`modelRegistry`, so passing any one of them here means
 * passing every one of those three explicitly too (matching their own
 * constructor defaults) rather than only the trailing ones this function
