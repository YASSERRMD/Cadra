 ...
 * is designed to be reusable for a later phase's deterministic headless
 * capture mode (rendering off the main thread there too, without a visible
 * canvas at all).
 *
 ...
 * required. `createBestAvailableRenderer` picks between the two
 * automatically, based on whether `OffscreenCanvas`/
 * `transferControlToOffscreen` are available in the current environment.
 *
 ...
 * the worker's global scope. `./worker-protocol.js` defines the typed
 * request/response message shapes both sides speak, and
 * `./scene-state-diff.js` implements the positional layer-diffing
 * optimization `createWorkerRenderer`/`worker-host.js` use to avoid
 * re-sending unchanged scene-node subtrees every frame.
 */
